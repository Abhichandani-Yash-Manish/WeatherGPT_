...
from .adapters import FORECAST,MARINE
...
def request_parameters(spec):
    ...
    if spec['product']=='river':params['daily']='river_discharge'
        ...
        params.update(hourly=','.join(FORECAST if spec['product']=='forecast' else MARINE),timezone='UTC',timeformat='unixtime')
        if spec['product']=='forecast':params.update(temperature_unit='celsius',wind_speed_unit='kmh',precipitation_unit='mm')
...
class IngestionDB:
    ...
    def enqueue(self, product, latitude, longitude, days, cycle_at, max_attempts=4):
        if product not in PRODUCTS: raise ValueError('Only forecast, marine and river jobs supported')
    ...
    def claim(self, lease_seconds=120):
        ...
        with self.transaction():
            ...
            row = self.db.execute("SELECT * FROM jobs WHERE state IN ('pending','retry') AND due<=? AND expires>? ORDER BY due,id LIMIT 1",(now,now)).fetchone()
    ...
    def complete(self, job, result):
        ...
        expected = spec['days']*variables*(1 if spec['product']=='river' else 24)
        ...
        start = parsed(spec['request_date']+'T00:00:00Z'); end = start+timedelta(days=spec['days'])
        times = [r.get('period_start_utc') if spec['product']=='river' else r.get('valid_time_utc') for r in result['records']]
        expected_times = {stamp(start+timedelta(hours=i)) for i in range(spec['days']*24)} if spec['product']!='river' else {stamp(start+timedelta(days=i)) for i in range(spec['days'])}
        if {stamp(parsed(t)) for t in times} != expected_times: raise ValueError('Result interval differs from fixed job date')
        encoded = canonical(result)
        receipt = {'product':family,'source_id':sid,'requested_point':requested,
    ...
    def latest(self, stream):
        ...
        if row is None: return {'status':'unavailable','operational_eligible':False}
        result = json.loads(row['result'])
        if identity(result)!=row['sha256']: raise ValueError('Published payload hash mismatch')
        latest=dict(self.db.execute('SELECT id,state,cycle,last_error FROM jobs WHERE stream=? ORDER BY cycle DESC LIMIT 1',(stream,)).fetchone())
        latest['last_error']=json.loads(latest['last_error']) if latest['last_error'] else None
        state='prototype_snapshot'
        if latest['cycle']>row['cycle'] and latest['state'] in {'retry','failed','expired'}:state='refresh_failed_or_missed'
        if epoch(self.clock())>=row['expires']:state='request_date_expired'
        return {'status':state,'latest_collection_job':latest,
...
def run_one(database, raw_root, opener=None):
    job=database.claim()
    ...
    try:
        if database.clock().astimezone(timezone.utc).date().isoformat()!=spec['request_date']:
            raise SourceError('Fixed request date no longer matches the provider relative-date API')
        result=getattr(Foundation(store),spec['product'])(spec['latitude'],spec['longitude'],spec['days'],refresh=True)
...
def backup(database, raw_root, output):
    ...
    try:
        with sqlite3.connect(stage/'ingestion.sqlite') as dest:database.db.backup(dest)
        with sqlite3.connect(stage/'ingestion.sqlite') as db:
            ...
            for row in db.execute('SELECT result,sha256 FROM versions'):
                ...
                rel=Path(meta['ingestion_attempt']['relative_root'])/meta['blob']
        ...
        (stage/'backup-manifest.json').write_text(json.dumps({'created_at':stamp(database.clock()),'files':files,
...
def restore(bundle, output):
    ...
    try:
        for rel,sha in manifest['files'].items():
            path=Path(rel)
            if path.is_absolute() or '..' in path.parts:raise ValueError('Unsafe backup member')
            src=bundle/path
            body=src.read_bytes()
        ...
        with sqlite3.connect(stage/'ingestion.sqlite') as db:
            if db.execute('PRAGMA integrity_check').fetchone()[0]!='ok':raise ValueError('Restored database integrity failed')
