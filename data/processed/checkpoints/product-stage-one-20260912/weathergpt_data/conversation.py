...
"""
import json,re,sqlite3,time,uuid,threading
...
class ConversationEngine:
    ...
    def _ask(self,body,q):
        ...
        if body.get('selection_id'):
            ...
            if state.get('last_plan'):
                history.append({'role':'assistant','content':json.dumps({'previous_interpretation':state['last_plan'],'source_resolved_places':state.get('resolved_points',{})},ensure_ascii=False)})
            plan,meta=self.model.plan(q,self.workspace.clock(),history);result['trace']['planning']=meta
            ...
            for p in plan['places']:
                ...
                if previous and not plan.get('clarification'):
        ...
        if result['expires_at_utc'] and parsed(result['expires_at_utc'])<=self.workspace.clock():
            result.update(status='stale',facts=[],citations=[],charts=[],calculations=[],answer='The retrieved forecast expired while the response was being prepared. Please ask again for a fresh answer.')
            for task in result.get('task_results',[]):
                task.update(status='stale',answer='This mixed response expired during preparation; ask again to retrieve verified evidence.',fact_ids=[])
        ...
        state['last_question']=q;state['last_plan']=plan;state['choices']=result['choices'];state['resolved_points']=result.get('resolved_points',{})
    ...
    def forecasts(self,result,plan,resolved,coordinates):
        now=self.workspace.clock();places=plan['places']
        if coordinates is not None:
            ...
            if places and any(p['name'].casefold() not in {'selected point','my location','here'} for p in places):
                result.update(answer='You supplied a pin and a different named place. Ask about “selected point” to use the pin, or remove it to search the name.');return result
            points=[{'name':'selected point','label':'selected point','coordinates':coordinates}]
        else:
            if not places:result.update(answer='Which village, town or city should I check? Add its state if the name is shared.',follow_up='Place name and state, or an explicit pin');return result
            points=[]
            ...
            for p in places:
                ...
                if p['kind'] in {'district','state','country','relative'}:
                    result.update(answer='Which village or town '+('near '+p['name'] if p['kind']=='relative' else 'within '+p['name'])+' should I check? I can retrieve a forecast for a precise place; I cannot yet give a verified map of weather across that whole area.',follow_up='Name a village/town and state, or supply a pin.');return result
                ...
                if not matches:
                    result.update(status='needs_clarification',answer=f"I could not find a settlement named {p['name']}"+(f" in {p['state']}" if p['state'] else '')+'. Please give its district/state, an alternative spelling, or a pin. I will not replace it with a nearby city.',follow_up='District/state, alternative spelling, or coordinates');return result
                ...
                if len(matches)>1 or matches[0]['match_type']=='approximate_name_requires_confirmation':
                    result.update(status='needs_selection',answer=f"I found {len(matches)} possible places for {p['name']}. Please confirm the intended location and spelling.",choices=matches[:20],follow_up='Choose a place, or add its district and state.');return result
                ...
                if a['status'] in {'prototype_answer','partial'} and (a.get('freshness') or {}).get('snapshot_status')=='prototype_snapshot':
                    ...
                    for v in a['values']:
                        ...
                        fact={'id':'f'+str(len(result['facts'])+1),'label':label,'value':value,'unit':v['unit'],'place':place['label'],
                              ...
                              'parameter':v['parameter'],'entity_id':place.get('selection_id') or json.dumps(place['coordinates'],sort_keys=True),
