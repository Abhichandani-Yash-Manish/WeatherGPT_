...
OPERATIONS=['lookup','compare','series','trend','daily','timeline','onset']
...
def validate_tasks(tasks,places):
    ...
    cells=0
    for t in tasks:
        if not isinstance(t,dict) or set(t) not in (FIELDS,FIELDS|{'request_quote'}):raise SourceError('Invalid task schema')
