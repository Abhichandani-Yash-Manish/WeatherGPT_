...
DIALOGUE_REQUEST_SCHEMA=obj({**REQUEST_SCHEMA['properties'],'context_action':{'type':'string','enum':['new','follow_up','correction','clarification_answer','explain_previous']},'changed_fields':{'type':'array','items':{'type':'string','enum':['places','time','parameters','operation','language','crop','growth_stage','topic']}}})
...
def validate_plan(plan):
    ...
    if 'changed_fields' in plan and (not isinstance(plan['changed_fields'],list) or not set(plan['changed_fields'])<={'places','time','parameters','operation','language','crop','growth_stage','topic'}):raise SourceError('Invalid context change fields')
