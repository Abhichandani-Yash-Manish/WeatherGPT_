...
def temporal_support(records):
    ...
    for parameter,rows in grouped.items():
        ...
        if aggregation in {'preceding_hour_sum','daily_value'}:
            start_key,end_key=('interval_start_utc','interval_end_utc') if aggregation=='preceding_hour_sum' else ('period_start_utc','period_end_utc')
            ...
            if aggregation=='preceding_hour_sum' and any(end!=t for (_,end),t in zip(spans,times)):
