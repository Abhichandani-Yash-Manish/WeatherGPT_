...
def lookup_plan(plan):
    ...
    if place['kind']=='country' and name.casefold() in {'india','all india','all-india','bharat'}:
        ...
        database=ROOT/products[sid]['processing_outputs']['directory']/'climate.sqlite'
        ...
        return {'status':'answered' if r['value_decimal'] is not None else 'unavailable','text':text,'facts':[{'label':plan['history_parameter'],'value':r['value_decimal'],'unit':r['unit'],'year':year,'period':period,'source_id':sid}],
    ...
    database=ROOT/products['S27']['processing_outputs']['directory']/'districts.sqlite'
    ...
    return {'status':'answered' if data['value_decimal'] is not None else 'unavailable','text':text,'facts':[{'label':'rainfall','value':data['value_decimal'],'unit':'mm','year':year,'period':period,'source_id':'S27'}],
