...
class ConversationEngine:
    ...
    def _ask(self,body,q):
        ...
        if body.get('selection_id'):
            ...
            plan,meta=self.model.plan(q,self.workspace.clock(),state['history']);result['trace']['planning']=meta
        result['plan']=plan
        ...
        if result['expires_at_utc'] and parsed(result['expires_at_utc'])<=self.workspace.clock():
            result.update(status='stale',facts=[],citations=[],answer='The retrieved forecast expired while the response was being prepared. Please ask again for a fresh answer.')
