...
def catalog(store,state=None,language="en"):
    ...
    def parse(raw,meta):
        ...
        if not parser.options or any(not p['label'] for p in parser.options):raise SourceError('Advisory selector schema changed or no entries supplied')
        if len({p['id'] for p in parser.options})!=len(parser.options):raise SourceError('Duplicate advisory selector identity')
        return parser
    raw,meta=store.fetch('S57',route if state else catalogue,{'s':state,'step1':'true'} if state else None,ttl=86400,product_validator=parse)
    parser=parse(raw,meta)
...
def document(store,state,district,language="en"):
    ...
    def parse_selection(raw,meta):
        ...
        return url
    raw,selection=store.fetch('S57',route,{'s':district,'step2':'true'},ttl=3600,product_validator=parse_selection)
    url=parse_selection(raw,selection)
    ...
    body,meta=store.fetch('S57',url,ttl=3600,max_bytes=20_000_000,product_validator=lambda b,m:pdf_pages(b))
    records=[{**p,'district_requested':district,'state_requested':state} for p in pdf_pages(body)]
