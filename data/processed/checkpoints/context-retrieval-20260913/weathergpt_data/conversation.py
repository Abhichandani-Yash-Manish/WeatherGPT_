...
class ConversationEngine:
    ...
    def _ask(self,body,q):
        ...
        if result['expires_at_utc'] and parsed(result['expires_at_utc'])<=self.workspace.clock():
            result.update(status='stale',facts=[],citations=[],charts=[],calculations=[],airport_reports=[],passages=[],document_evidence=[],answer='The retrieved evidence expired while the response was being prepared. Please ask again for a fresh answer.')
