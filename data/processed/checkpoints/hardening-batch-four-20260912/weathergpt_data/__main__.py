...
def main():
    ...
    q=sub.add_parser('answer',help='Answer an explicit point-forecast question from stored evidence; no provider calls')
    ...
    try:
        ...
        if cmd=='build-climate':result={'directory':str(build())}
            ...
            result=AnswerService(args.database,args.raw_root,args.geography_database).answer(
                args.question,timezone_name=args.timezone,entity_id=args.entity_id,coordinates=coordinates)
        else:
