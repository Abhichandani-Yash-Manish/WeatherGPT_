...
from .adapters import FORECAST, temporal_support
...
def calculate(records, parameter, start, end):
    ...
    unit,aggregation,_,_=FORECAST[parameter]
...
class AnswerService:
    ...
    def _select_and_answer(self,db,response,start,end,now):
        ...
        response['freshness']={'retrieved_at_utc':meta['retrieved_at_utc'],'retrieval_age_seconds':snapshot['retrieval_age_seconds'],
                               ...
                               'refresh_health':snapshot['refresh_health'],'snapshot_status':snapshot['status'],
                               'next_planned_collection':snapshot['next_planned_collection'],
        ...
        if self.raw_root not in raw.parents or digest(raw.read_bytes())!=meta['sha256']:
        ...
        if snapshot['status']!='prototype_snapshot':
            response['limitations'].append('A newer due collection is pending, missed or failed; the last published snapshot is being shown.')
        ...
        if response['status']=='degraded':response['answer']+=' A newer due refresh is incomplete; this is the last published snapshot.'
