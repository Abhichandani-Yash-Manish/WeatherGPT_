...
def save_focus(state,result):
    ...
    state['dialogue_state']={'version':1,'plan':state.get('last_plan',result['plan']) if result['plan'].get('context_action')=='explain_previous' else result['plan'],'status':result['status'],
      ...
      'evidence_expiry':result['expires_at_utc']}
    # Re-explanations preserve the exact prior packet, source binding and expiry.
    if result['facts'] or result.get('airport_reports'):state['last_evidence']={k:copy.deepcopy(result[k]) for k in ['facts','citations','expires_at_utc','answered_at_utc','notes','plan','status','answer','airport_reports','charts','calculations','task_results','task_coverage','retrieval_plan'] if k in result}

    if not result['facts'] and not result.get('airport_reports'):state.pop('last_evidence',None)
...
def ground_relative_slots(plan,question,now):
    ...
    for t in plan['tasks']:
        if t['kind'] not in {'forecast','history'} or t['start_local'] or t['end_local'] or plan['explicit_times']:continue
