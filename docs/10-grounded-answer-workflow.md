...
1. Semantic publication checks for geocoding, CAP and document routes are now implemented and tested in the [extensive validation batch](11-extensive-validation-and-rag-gate.md), alongside the restricted `rag-context` entry point. Continue with held-out generated-answer evaluation.
