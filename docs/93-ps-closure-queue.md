...
| Python suite | 1338 tests collected and passing, which is the count the README records (1309 before the QA repair batch of 17 September, docs/96) | `python3 -m pytest tests/ -q` |
| React suite | 59 suites, 329 component checks, `tsc --noEmit` clean (55 and 296 before docs/96; docs/97 added the dashboard suite; the aurora-glass overhaul of docs/98 added the kit suite; the reported-defect repairs of docs/99 added the matrix geometry checks) | `frontend/`, run under Vitest |
