...
## Round 19 — the district corpus becomes reachable, and the topic word decides

A fourteen-question probe over the published-corpus route found two exceptions, two questions that asked for a place the question had already given, three district crop questions ending in an internal mismatch, and one district edition served for a question about grapes with pearl-millet and paddy passages. The district family was indexed under 571 districts and could not be requested: `FAMILIES` held the discovery-address families and the target-driven district family lived only in `DISTRICT_SPEC`, so task validation refused it. That is now one registry view (`ALL_FAMILIES`); `printed_validity` builds a timezone instead of passing a UTC offset as `tzinfo`; an absent state or district answers as an absence naming what is held; every name the place record supplies is tried against the publisher’s directory and a rejected label such as a revenue division is disclosed; the indexed edition answers with the live refusal as its first sentence when the live reader cannot verify the current PDF; and `topic_tokens` removes the place names and the words every published document shares so the topic word decides. Measured after: 11 of 14 answered, 1 partial, 2 honest absences, 0 exceptions; four of four district crop questions answer. docs/64 carries the table, the evidence and the limits.

## Round 20 — reading the question in every language we can write

The workspace could write 21 Indian scripts and could not read a day, a part of day, a measure or a place name in most of them. "How will the weather be tomorrow morning?" in Hindi took 12.6 s through a model and came back with 06:30-12:30 while the same English question planned 09:30-12:30, because `dialogue.ground_relative_slots` carried its own morning band; a Gujarati "tomorrow" question read no day word at all (the table held only the formal form); Urdu, Marathi, Assamese, Sindhi, Nepali and Odia rain questions had no rules plan; and Bengali, Gurmukhi, Odia, Arabic-script and inflected native-script place names read no place at all. Tables are now held per language, one definition per part of day is shared by the rules floor, the grounding compiler and a new settle step for model-written windows, and the place class and marker table cover every script the product writes, longest marker first. Measured after: 18 of 23 languages read a day and every part of day (five declared unread by name), 18 have all four core measures, and the eight-language journey set answered six with every window read from the question. docs/65 carries the tables and the limits.

## Round 21 — a question in one language, documents in another

The corpus is printed in English and the reader may write in any script the product writes. Before this round a Hindi bulletin question was planned as a rainfall forecast of a settlement called "बारे", a Bengali one was answered by asking which settlement "জাতী" was, and a Gujarati district question reported that no edition was indexed under the name the reader used. Now a question in an Indic script is translated for retrieval only when no passage shares an exact word with it; the passages served are the source’s own text, the translated wording is recorded, and the answer says which language was translated to English and that the reading is partial. The planner holds the product words per language, compares words on their folded form (Bengali writes য় two ways), refuses postpositions as place names, reads a name followed by its unit word as a district or state, and tries the publisher’s English name before the reader’s spelling. Six measured questions in Hindi, Gujarati, Marathi, Bengali and Tamil went from none answered to six answered or partial through the translation, with no exception. docs/67 carries the table and the limits, including 21-52 seconds per question and no translation-quality review.

## Round 22 — a translated answer that does not stall

A Gujarati district question took 27.4 s, of which about 22 s was the composed answer being translated sentence by sentence — eleven service calls, most of them spent on quoted passages. Now a source quotation is never translated (it is the document’s own words, and a rendering would put a paraphrase where the reader expects the source), the prose goes through the unchanged gate in a bounded pool of four so the wall clock is the slowest call rather than the sum, a transient service failure is retried once while a damaged value never is, and the rendering of a repeated sentence or question is served from a bounded local render cache with hits counted. Measured after: 8.7 s cold and 1.2 s repeated for the same question, 4.0-14.4 s cold and 1.0-7.7 s repeated across three questions, with no rendering downgraded. docs/69 carries the table and the limits, including that adherence is not quality and that a quotation stays in the document’s printed language.
