...
def explain_evidence(result):
    if result.get('passages'):
        return 'These are passages from the published district bulletin, matched to its crop and stage labels. The publication date and forecast context are shown separately. They do not establish your field conditions or grant a personal go/no-go decision. Open the full source passage to retain every condition and unit.'
