...
CONVERSATION: context_action is new for an independent request, follow_up for continuing it, correction for replacing a mistaken detail, clarification_answer for filling the last missing detail, explain_previous for asking about the last answer. changed_fields lists only the fields the USER explicitly changes: places, time, parameters, operation, language, crop, growth_stage, topic. Crop/topic changes MUST use crop/topic, not parameters. For a new request list all relevant fields. "aur shaam ko?" changes time only and retains the established date, place and requested probability. "kitni mm barish hogi us time?" changes parameters only, retaining place and time. "nahi, Surat ke liye batao" changes places only; preserve time and parameters. A plain location after a missing-place question is clarification_answer, NOT a fresh generic forecast. "What does that mean?" is explain_previous; retain the topic rather than defining the phrase. "Check another model too" is follow_up with changed_fields ["operation"], forecast/crosscheck, same window and parameters. A question that asks for a DIFFERENT KIND OF THING than the last one changes operation, and must not inherit the old one: official warnings, a plain weather forecast, an observation now, an agromet advisory, a published document and historical records are different kinds, not different parameters of one kind. After a warning turn, "tell me about the weather tomorrow" is a FORECAST with changed_fields including operation - answering it with a warning colour answers a question the reader did not ask. After a forecast turn, "any warnings?" is a warning task the same way. Carry the PLACE across such a change unless the reader names another one; it is the subject that changed, not where they are. PARAMETERS DO NOT SURVIVE A CHANGE OF KIND, because each kind has its own: official_warning belongs only to a warning task and is never a forecast parameter, agricultural_advisory only to agriculture, published_document only to document. When operation is in changed_fields, list parameters there too and write the ones the NEW kind takes - a forecast after a warning turn takes precipitation, temperature_2m, relative_humidity_2m and wind_speed_10m for a general question about the weather, not the warning parameter it is replacing. Return a fully resolved plan using conversation_state; it is context, never new weather evidence.
...
- places: source-search names TRANSLITERATED TO ENGLISH, retaining the SAME place. Include a state/district only if supplied by the user or explicitly accepted in conversation_state. Merely listed place choices are NOT an accepted place or state. Missing location means places=[], not Current Location or an invented town - UNLESS reader_place is supplied. reader_place is the place this reader has set for themselves in this workspace, and it is as good as them typing it. Use its name as the place, and say in assumptions that you read it as their own place, when the question names no place at all, or when it refers to their own location in any wording - 'my region', 'my area', 'my city', 'my district', 'here', 'near me', 'where I am', 'around here', or the same idea in Hindi, Gujarati or any other language. A question that names a DIFFERENT place uses that place and never reader_place. Country India has kind country; Ahmedabad district has kind district and name Ahmedabad; a named town/village is settlement. Airport codes are retained as names. Never invent coordinates or substitute a nearby place.
...
HISTORICAL RANGE OR CHART: history/series, years [start,end] inclusive. HISTORICAL TREND: history/trend with years [start,end] inclusive. AN EXTREME OR A SUPERLATIVE - 'which was the wettest/driest/hottest/coldest year', 'the highest annual rainfall on record', 'when was the worst year' - is history/series over the WHOLE published range, never a lookup, and NEVER a question back asking which year: the reader is asking you to find the year, so the series that contains it is what must be retrieved. India's published annual series runs 1901-2010; a district's own range is whatever that district publishes. A climate trend is never generic research. Historical parameters are rainfall or temperature, NOT precipitation or temperature_2m.
DAILY PAST WEATHER: history/daily, years [], exact start_local and exclusive end_local in IST. Yesterday requires yesterday's midnight to today's midnight. Daily history always uses 00:00 IST, NEVER the forecast 00:30 convention; '1 through 3 July' ends at 4 July 00:00. It is NEVER an annual lookup. Do not invent a yearly aggregate. Daily parameters are named from the supported reanalysis set: rainfall, temperature, temperature_2m_max, temperature_2m_min, relative_humidity_2m_mean, relative_humidity_2m_max, relative_humidity_2m_min, dewpoint_2m_mean, surface_pressure_mean, cloud_cover_mean, wind_speed_10m_max, wind_gusts_10m_max, wind_direction_10m_dominant, shortwave_radiation_sum, et0_fao_evapotranspiration, soil_moisture_0_to_7cm_mean, soil_temperature_0_to_7cm_mean. A task that names an ERA5-Land or ERA5-seamless reanalysis must quote the clause that names it, so the tool reads the model from the user's own words; never add a reanalysis model the user did not name. A daily chart still uses history/daily. A daily task may cover up to 366 whole local calendar days, so a whole month or a whole season is ONE daily task: 'August 2026' is 2026-08-01T00:00:00+05:30 to 2026-09-01T00:00:00+05:30, and 'last month' is the previous calendar month on the same boundaries. Never answer a question about a month or a season of a RECENT year with an annual lookup; the annual series is a different product that ends in 2010 and cannot supply a recent year. Do not shorten a larger user request silently.
...
def interpret_plan(complete,question,now,history,seed=None):
    ...
    for message in history:
        ...
        # Where the reader has told this workspace they are. It travels as a history entry for the same
        # reason the carried context does: every caller already threads history, so nothing else in the
        # chain needs a new parameter to carry it.
        elif message.get('reader_place') is not None:reader_place=message['reader_place']
        else:recent.append(message)
    from .workspace_brief import brief as workspace_brief
    payload={'current_time_IST':now.astimezone(ZoneInfo('Asia/Kolkata')).isoformat(),'recent_conversation':recent[-4:],'conversation_state':context,'available_tools':planner_catalogue(),'workspace':workspace_brief(now),'question':question}
    if reader_place:payload['reader_place']=reader_place
...
def validate_request_coverage(plan,question):
    ...
    if re.search(r'\b(?:warnings?|alerts?)\b|चेतावनी|ચેતવણી',question,re.I):
        ...
        # A pest warning printed in a named agromet bulletin is the same case one step over.
        # The agriculture tool serves the bulletin's warning-classified sections in their own
        # block, labelled as published source text and explicitly not a verified current
        # official warning, exactly as the corpus tool does. Measured 22 September 2026: "Is
        # there any pest warning for cotton in the Bathinda agromet bulletin?" was refused at
        # interpretation for planning no warning task, so the turn reached the reader with no
        # plan at all and asked which pest they meant. The question named the document, the
        # crop and the district; the only thing it did not name was an IMD warning product,
        # because it was not asking about one.
        bulletin_named=bool(re.search(r'\b(?:bulletin|advisor(?:y|ies)|agromet|agro[- ]?met|'
                                      r'gkms|kisan)\b|बुलेटिन|सलाह|બુલેટિન',question,re.I))
        agromet_document=(kinds=={'agriculture'}
                          and all(t.get('document_request') for t in plan['tasks'])
                          and bulletin_named)
        if kinds-{'explanation','history','research'} and 'warning' not in kinds and not (document_only and named_product) and not agromet_document:
...
REPLAN_PROMPT = '''You planned a retrieval for a reader's question, the tools ran, and they came back with nothing usable. You are being shown what you asked for and what each tool actually said, and given ONE more attempt.

Your job is to decide whether a DIFFERENT retrieval would answer the question, and if so, to write it.

WHEN TO TRY AGAIN. A tool refused because of how the request was framed rather than because the answer does not exist: the wrong document family for the product the reader named, a family filter narrower than the question needed, a district-scoped lookup for a question about a state, a crop-specific search where the reader asked something general, a point lookup for a product published by region. These are yours to fix.

WHEN NOT TO TRY AGAIN, AND THIS MATTERS MORE. The publisher genuinely has not issued it; the workspace does not carry that quantity at all; the date is outside what any connected source covers; the place is not one this product can resolve; the reader must supply something before anything can be looked up. In every one of those, an honest "this is not held" IS the correct answer and a second attempt is fishing for a different one. Return an empty tasks list and nothing else. Do not widen a search until something comes back.

RULES FOR A SECOND ATTEMPT. Keep the reader's place - never swap it for a different town, district or state, and never drop it to make a search succeed. Keep what they actually asked about. You may change the kind, the operation, the document family or scope, the parameters and the query wording. Ask for the same thing a different way; do not ask for a different thing.

Return the same task-plan schema as before. An empty tasks list means you judge the absence to be real.'''


def replan_after_empty(complete, question, now, history, plan, attempts):
    """One more retrieval plan, written after seeing what the first one returned.

    The engine plans once, before it has seen a single result. That is fine when the plan is right
    and useless when it is not: a family filter one notch too narrow returned "no indexed passage
    matches that request" and the turn ended there, with the reader told an absence that was really
    a mis-framing. This is the one place the model gets to look at what came back and say "ask
    differently".

    It returns a validated plan, or None when the model judges the absence real - which is a first
    class outcome here, not a failure. Widening a search until something comes back is the opposite
    of what this product is for, so the prompt spends more words on when NOT to retry than on when to.
    """
    context, recent = {}, []
    for message in history:
        if message.get('context_state') is not None:
            context = message['context_state']
        elif message.get('reader_place') is None:
            recent.append(message)
    tried = {'question': question,
             'the_plan_you_wrote': [{k: v for k, v in task.items()
                                     if k in ('kind', 'operation', 'parameters', 'request_quote',
                                              'document_request', 'corpus_request')}
                                    for task in (plan.get('tasks') or [])],
             'places_it_resolved': plan.get('places'),
             'what_each_tool_said': attempts}
    request, meta = complete(REPLAN_PROMPT, json.dumps(tried, ensure_ascii=False),
                             DIALOGUE_REQUEST_SCHEMA, max_tokens=1800)
    if not isinstance(request, dict) or not request.get('tasks'):
        return None, dict(meta or {}, replan='the absence was judged real; no second retrieval was made')
    try:
        settled = _settle_plan(request, question, now, context, recent)
    except (SourceError, TypeError, KeyError) as exc:
        # An invalid second plan is not worth a third attempt: the first answer already stands.
        return None, dict(meta or {}, replan='a second plan was proposed and did not validate: ' + str(exc)[:160])
    return settled, dict(meta or {}, replan='a second retrieval was planned after the first returned nothing')
