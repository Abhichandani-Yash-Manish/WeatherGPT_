...
class WarningHeadlineTests(unittest.TestCase):
    ...
    def test_an_absent_summary_is_unknown_not_a_quiet_claim(self):
        # A payload thinner than the current backend must not read as "no warning".
        view = {'data': {'days': []}}
        data = view['data']
        known = 'has_hazard' in data
        self.assertFalse(known, 'the frontend must treat an absent summary as unknown')
