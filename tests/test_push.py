...
class PushStoreTests(unittest.TestCase):
    ...
    def test_browser_millisecond_expiry_is_normalised_and_blocks_delivery(self):
        from weathergpt_data.push import deliverable_channels
        expires = int(NOW.timestamp() * 1000)
        self.store.subscribe(ENDPOINT, P256DH, AUTH, watch_id=self.watch['id'], expires_at=expires, now=NOW)
        channels, skipped = deliverable_channels(self.path, self.watches.get(self.watch['id']), now=NOW)
        self.assertNotIn('web_push', channels)
        self.assertTrue(skipped)
        self.assertEqual(self.store.list(state='expired')[0]['expires_at'], '2026-09-16T00:00:00+00:00')

    def test_unsubscribing_one_browser_keeps_another_browsers_channel(self):
        self.store.subscribe(ENDPOINT, P256DH, AUTH, watch_id=self.watch['id'], now=NOW)
        self.store.subscribe(ENDPOINT + '-second', P256DH, AUTH, watch_id=self.watch['id'], now=NOW)
        self.store.unsubscribe(ENDPOINT, now=NOW)
        self.assertIn('web_push', self.watches.get(self.watch['id'])['channels'])
...
class TtlTests(unittest.TestCase):
    def test_ttl_follows_the_window_end_with_bounds(self):
        ...
        self.assertEqual(push_ttl(past, now=NOW), 0)
